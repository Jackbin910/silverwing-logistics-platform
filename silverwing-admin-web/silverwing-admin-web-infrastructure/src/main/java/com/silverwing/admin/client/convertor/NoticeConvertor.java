package com.silverwing.admin.client.convertor;

import com.silverwing.admin.application.command.SaveNoticeCommand;
import com.silverwing.admin.application.dto.NoticeReadUserResponse;
import com.silverwing.admin.application.dto.NoticeResponse;
import com.silverwing.biz.notice.domain.model.aggregate.SysNoticeAggregate;
import com.silverwing.biz.notice.domain.model.entity.NoticeReadUserEntity;
import org.mapstruct.Mapper;
import org.mapstruct.Mapping;
import org.mapstruct.MappingTarget;
import org.mapstruct.NullValuePropertyMappingStrategy;

/**
 * 通知公告命令 / 聚合根 / 响应互转（防腐层）。
 */
@Mapper(componentModel = "spring", nullValuePropertyMappingStrategy = NullValuePropertyMappingStrategy.IGNORE)
public interface NoticeConvertor {

    /** 新增场景：命令转换为聚合根（忽略主键，由数据库生成） */
    @Mapping(target = "id", ignore = true)
    SysNoticeAggregate toEntity(SaveNoticeCommand command);

    /** 更新场景：将命令字段合并到已有聚合根（忽略主键，保留原有主键） */
    @Mapping(target = "id", ignore = true)
    void applyCommandToEntity(@MappingTarget SysNoticeAggregate entity, SaveNoticeCommand command);

    /** 聚合根转换为响应体 */
    NoticeResponse toResponse(SysNoticeAggregate aggregate);

    /** 已读用户领域模型转换为响应体 */
    NoticeReadUserResponse toReadUserResponse(NoticeReadUserEntity entity);
}
